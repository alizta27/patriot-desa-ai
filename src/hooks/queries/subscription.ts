...
export const useCreateSubscription = () => {
  ...
  return useMutation({
    mutationFn: async (data: { user_id: string; customer_name: string; customer_email: string }) => {
...
export const useCheckSubscription = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, userId) => {
      ...
      if (data.expired) {
        toast.info('Subscription Anda telah berakhir dan telah diturunkan ke paket gratis');
      }
    },
    onError: (error: any) => {
